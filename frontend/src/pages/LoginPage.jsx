...
/* ── glow blob ── */
function GlowBlob({ x = '50%', y = '50%', color = LOGO_BLUE, size = 480, opacity = 0.07 }) {
  return (
    <Box sx={{
    ...
    <Box sx={{
      ...
      <Box sx={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 420 }}>
        ...
        <Card sx={{
          ...
          <Typography variant="h5" sx={{ color: '#e6edf7', mb: 0.5 }}>Sign in</Typography>
...
/>
            ...
            <Box sx={{ textAlign: 'right', mt: -1 }}>
              <Typography component="button" type="button" onClick={() => {}}
                sx={{
                  fontSize: 12.5, color: LOGO_BLUE, background: 'none', border: 'none',
                  cursor: 'pointer', p: 0, fontFamily: 'inherit',
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Typography sx={{ fontSize: 13.5, color: '#5b6d8a' }}>
              New to TrackFlow?{' '}
              <Typography component="span" onClick={onRegister}
                sx={{
                  color: LOGO_BLUE, fontWeight: 600, cursor: 'pointer',
                  '&:hover': { textDecoration: 'underline' },
                }}>
                Create an account
              </Typography>
