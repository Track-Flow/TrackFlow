...
// Update IDs to match your departments table
const DEPARTMENTS = [
    ...
    <Box 
     ...
     sx={{
      ...
      <Box sx={{ flex: 1, minWidth: 0, pl: 1 }}>
        ...
        <Typography sx={{ fontSize: 13.5, fontWeight: 600, color: '#e6edf7', mb: 0.25 }} noWrap>{ticket.ticket_title}</Typography>
        <Typography sx={{ fontSize: 12, color: '#8fa2c0' }} noWrap>{ticket.ticket_description}</Typography>
      </Box>
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexShrink: 0 }}>
        <FormControl size="small" sx={{ minWidth: 150 }}>
          <Select value={deptId} onChange={e => setDeptId(e.target.value)} displayEmpty
            sx={{ fontSize: 12, color: '#e6edf7' }}>
            <MenuItem value="" disabled><em style={{ color: '#5b6d8a' }}>Select dept…</em></MenuItem>
            {DEPARTMENTS.map(d => <MenuItem key={d.id} value={d.id}>{d.name}</MenuItem>)}
          </Select>
        </FormControl>
        <Button size="small" variant="contained" disabled={!deptId || routing}
          onClick={handleRoute} sx={{ fontSize: 11, py: 0.5, whiteSpace: 'nowrap' }}>
      ...
      <Card sx={{ bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
        ...
        {!loading && tickets.length === 0 && (
          <Box sx={{ p: 6, textAlign: 'center' }}>
            ...
            <Typography sx={{ color: '#8fa2c0', fontSize: 13, mt: 0.5 }}>All tickets assigned.</Typography>
          </Box>
        )}
        {!loading && tickets.map(t => <TicketRow key={t.ticket_id} ticket={t} onRoute={handleRoute} />)}
      </Card>
    </Box>
  );
}