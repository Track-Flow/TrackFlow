...
import { getMyTickets, getUnassigned, getSLABreaches, statusMeta, priorityMeta, timeAgo } from '../helpers/ticketHelpers';
...
function KpiCard({ label, value, icon, color, sub, subUp }) {
  return (
    <Card sx={{
      flex: 1, p: 2.5, bgcolor: PAPER, border: `1px solid ${BORDER}`,
      position: 'relative', overflow: 'hidden',
    }}>
      <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, bgcolor: color }} />
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
        <Typography sx={{ fontSize: 11, color: '#8fa2c0', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>
          {label}
        </Typography>
        <Box sx={{
      ...
      <Typography sx={{ fontFamily: '"Rubik", sans-serif', fontSize: 36, fontWeight: 700, color: '#e6edf7', lineHeight: 1 }}>
        {value}
      </Typography>
      {sub && (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.75 }}>
          <span className="material-symbols-outlined" style={{ fontSize: 13, color: subUp ? '#2bd48f' : '#ff6b6b' }}>
            {subUp ? 'trending_up' : 'trending_down'}
          </span>
          <Typography sx={{ fontSize: 11.5, color: '#5b6d8a' }}>{sub}</Typography>
        </Box>
      )}
    </Card>
  );
...
function FilterChip({ label, count, active, onClick }) {
  return (
    <Box onClick={onClick} sx={{
      ...
      transition: 'all .15s',
      '&:hover': { border: `1px solid ${ACCENT}`, bgcolor: 'rgba(46,200,255,0.07)' },
    }}>
      <Typography sx={{ fontSize: 12.5, fontWeight: active ? 700 : 500, color: active ? ACCENT : '#8fa2c0' }}>
        {label}
      </Typography>
      {count !== undefined && (
        <Box sx={{
          px: 0.75, borderRadius: 999, fontSize: 11, fontWeight: 700,
...
const initials = getInitials(ticket.user_name ?? ticket.user_id ?? '??');
  ...
  return (
    <Box sx={{
      ...
      p: 2, borderBottom: `1px solid ${BORDER}`,
      position: 'relative',
      '&:hover': { bgcolor: 'rgba(46,200,255,0.02)' },
    }}>
      {/* Priority left bar */}
      <Box sx={{ position: 'absolute', left: 0, top: 12, bottom: 12, width: 3, borderRadius: 2, bgcolor: pColor }} />

      {/* Avatar */}
      <Avatar sx={{ width: 36, height: 36, fontSize: 12, fontWeight: 700, bgcolor: `${pColor}22`, color: pColor, flexShrink: 0, ml: 0.5 }}>
        {initials}
      </Avatar>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.5, flexWrap: 'wrap' }}>
          <Typography sx={{ fontFamily: 'monospace', fontSize: 11, color: '#5b8ec2', fontWeight: 600 }}>
            {ticket.ticket_id}
          </Typography>
          ...
          <Box sx={{ flex: 1 }} />
          <Typography sx={{ fontSize: 11, color: '#5b6d8a' }}>{timeAgo(ticket.updated_at ?? ticket.created_at)}</Typography>
        </Box>
...
function PulseItem({ ticket, tlaName }) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1.5, mb: 2 }}>
      <Avatar sx={{ width: 32, height: 32, fontSize: 11, fontWeight: 700, flexShrink: 0,
        bgcolor: `${pColor}20`, color: pColor }}>
        {initials}
      </Avatar>
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography sx={{ fontSize: 12.5, color: '#e6edf7', lineHeight: 1.5 }}>
          <span style={{ fontWeight: 700 }}>{tlaName ?? ticket.assignee_id ?? 'A TLA'}</span>
          {' '}{action}{' '}
...
export default function TLAHome() {
  ...
  const handleUpdateStatus = async (id, status) => {
    try { await api.patch(`/api/tickets/${id}`, { ticket_status: status }); fetchTickets(); }
  ...
  // Filtered display
  const displayed = filter === 'mine'       ? myTickets
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" onClick={() => navigate('/tla/board')}
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 2.5, alignItems: 'start' }}>
        ...
        <Card sx={{ bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
          ...
          <Box sx={{ p: 2, borderBottom: `1px solid ${BORDER}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
              ...
              <Typography sx={{ fontSize: 10.5, color: '#5b6d8a', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>
                Real-time stream
              </Typography>
            </Box>
        ...
        <Card sx={{ bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
          <Box sx={{ p: 2, borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              <Typography sx={{ fontSize: 10.5, color: '#c084fc', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, mb: 0.25 }}>
                Activity
              </Typography>
          ...
          <Box sx={{ p: 2 }}>
            ...
            {!loading && teamPulse.map(t => (
              <PulseItem key={t.ticket_id} ticket={t} tlaName={t.assignee_id} />
            ))}
          </Box>
        </Card>
