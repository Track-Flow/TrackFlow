...
import { Box, Card, Typography, Button, Chip, CircularProgress, Alert } from '@mui/material';
import api from '../helpers/api';
import { getMyTickets, getUnassigned, statusMeta, priorityMeta, timeAgo } from '../helpers/ticketHelpers';
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ flex: 1, minWidth: 0, pl: 1 }}>
        ...
        <Typography sx={{ fontSize: 12, color: '#8fa2c0', mt: 0.25 }}>{ticket.user_id ?? 'Unknown'}</Typography>
      </Box>
      <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
        {!ticket.assignee_id && (
          <Button size="small" variant="contained" onClick={() => onClaim(ticket.ticket_id)}
            sx={{ fontSize: 11, py: 0.4 }}>Claim</Button>
        )}
        {isAssignedToMe && ticket.ticket_status === 'open' && (
          <Button size="small" variant="outlined" onClick={() => onUpdateStatus(ticket.ticket_id, 'in_progress')}
            sx={{ fontSize: 11, py: 0.4 }}>Start</Button>
        )}
        {isAssignedToMe && ticket.ticket_status === 'in_progress' && (
          <Button size="small" variant="outlined" color="success" onClick={() => onUpdateStatus(ticket.ticket_id, 'resolved')}
            sx={{ fontSize: 11, py: 0.4 }}>Resolve</Button>
        )}
      </Box>
    </Box>
  );
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        ...
        <KpiCard label="Resolved"        value={myTickets.filter(t => t.ticket_status === 'resolved').length}    color="#2bd48f"  />
      </Box>
      ...
      <Card sx={{ bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
        ...
        {!loading && displayed.map(t => (
          <TicketRow key={t.ticket_id} ticket={t} myId={user?.id}
            onClaim={handleClaim} onUpdateStatus={handleUpdateStatus} />
        ))}
      </Card>
    </Box>
  );
