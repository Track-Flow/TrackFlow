...
import { countByStatus, getSLABreaches, getResolvedToday, statusMeta, priorityMeta, timeAgo } from '../helpers/ticketHelpers';
...
function KpiCard({ label, value, color = ACCENT, sub }) {
  return (
    <Card sx={{ p: 2.5, flex: 1, bgcolor: PAPER, border: `1px solid ${BORDER}`, position: 'relative', overflow: 'hidden' }}>
      <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, bgcolor: color }} />
      <Typography sx={{ fontSize: 11, color: '#8fa2c0', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, mb: 0.75 }}>
        {label}
      </Typography>
      <Typography sx={{ fontFamily: '"Rubik", sans-serif', fontSize: 34, fontWeight: 700, color: '#e6edf7', lineHeight: 1 }}>
        {value}
      </Typography>
      {sub && <Typography sx={{ fontSize: 11.5, color: '#5b6d8a', mt: 0.5 }}>{sub}</Typography>}
    </Card>
  );
}
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: BG, p: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        <KpiCard label="Open tickets"    value={counts.open        ?? 0} color={ACCENT}   />
        <KpiCard label="In progress"     value={counts.in_progress ?? 0} color="#ffb547"  />
        <KpiCard label="Resolved today"  value={resolvedToday.length}    color="#2bd48f"  />
        <KpiCard label="SLA breaches"    value={breaches.length}         color="#ff6b6b"  sub={breaches.length > 0 ? 'Needs attention' : 'All on track'} />
      </Box>
      ...
      <Card sx={{ bgcolor: PAPER, border: `1px solid ${BORDER}` }}>
        <Box sx={{ p: 2, borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            ...
            <Typography variant="h6" sx={{ color: '#e6edf7' }}>Live feed</Typography>
          </Box>
          <Typography sx={{ fontSize: 12, color: '#5b6d8a' }}>{tickets.length} total</Typography>
        </Box>
